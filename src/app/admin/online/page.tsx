...
import {forceLogoutUsingPost1, getOnlineUsersUsingGet1} from "@/api/adminController";
...
const OnlinePage: React.FC = () =>
{
    ...
    const fetchUserData = async (params: any,
        ...
        const response = await getOnlineUsersUsingGet1({
            ...params,
            sortField,
            sortOrder,
            ...filter,
    ...
    const forceLogout = async (user: OnlineUser) =>
    {
        return await forceLogoutHandler(async () =>
        {
            const {code} = forceLogoutUsingPost1({
                userId: user.id
            })
            return code === 0;
        }, [])
